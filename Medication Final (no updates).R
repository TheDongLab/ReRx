...
for (drug in drug_patients$Active_Ingredients) {
  # Mark drug exposure for all matched subjects
  ...
  # Calculate counts for contingency table (used in results summary)
  ...
  # Fit conditional logistic regression only if drug_used has both 0 and 1
  if (length(unique(analysis_data$drug_used)) == 2) {
    model <- tryCatch(
      clogit(ALS_status ~ drug_used + SEX + exit_age + strata(strata), data = analysis_data),
